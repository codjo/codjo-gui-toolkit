...
public class TestUtil {
    ...
    public static void pressKey(Component component, Key key) {
        ...
        EventListener[] keyListeners = component.getListeners(KeyListener.class);
        if (keyListeners.length > 0) {
            KeyEvent event =
                  new KeyEvent(component, KeyEvent.KEY_PRESSED, 0, modifier, keyCode,
                               (char)keyCode);
            for (int i = 0; i < keyListeners.length; i++) {
                KeyListener keyListener = (KeyListener)keyListeners[i];
        ...
        if (JComponent.class.isInstance(component)) {
            ...
            final ActionListener actionForKeyStroke =
                  ((JComponent)component).getActionForKeyStroke(keyStroke);
            if (actionForKeyStroke != null) {
                actionForKeyStroke.actionPerformed(new ActionEvent(component,
    ...
    public static Component findComponentWithName(String name, Component component) {
        if (component == null
        ...
        for (int i = 0; i < components.length; i++) {
            Component subComponent = components[i];
